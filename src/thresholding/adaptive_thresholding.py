...
class AdaptiveThresholding(QWidget):
    ...
    This class provides a GUI for selecting an image and applying adaptive thresholding to it. The user is able to
    manually indicate a block size and a C constant with sliders. Users can view the original grayscale image and
    two binary images that use adaptive thresholding
    (cv2.ADAPTIVE_THRESH_MEAN_C & cv2.ADAPTIVE_THRESH_GAUSSIAN_C), as well as obtain computer-extracted text from any of
    these images. The user may also save the preprocessed binary image with a specific filename, image format, and
    directory. The user can also display and save the corresponding histogram graphs of these images as well.
    ...
    def extract_text(self):
        ...
        if method_idx == 1:
            image = cv2.adaptiveThreshold(
                self.image, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, self.block_size, self.c_constant,
            )
        elif method_idx == 2:
            image = cv2.adaptiveThreshold(
                self.image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, self.block_size, self.c_constant,
            )
        else:
            image = self.image
